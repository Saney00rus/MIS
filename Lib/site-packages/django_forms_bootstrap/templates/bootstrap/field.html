...
<div id="div_{{ field.auto_id }}"
     class="form-group {% if field.errors %}has-error{% endif %} {% if field.css_classes %} {{ field.css_classes }}{% endif %}">

    {% if field.label and not field|css_class == "checkboxinput" %}
    <label for="{{ field.auto_id }}"
           class="control-label {% if field.field.required %}required-field{% endif %} {{ css_classes.label }}">
        {{ field.label|safe }}
    </label>
    {% endif %}
    ...
    <div class="{{ css_classes.wrap }}">
        ...
        <div class="checkbox">
            <label>
                <input type="checkbox" name="{{ field.html_name }}" id="id_{{ field.html_name }}_{{ forloop.counter }}"
                       value="{{ choice.0 }}"
                       {% if choice.0 in field.value or choice.0|stringformat:"s" in field.value or
                choice.0|stringformat:"s" == field.value|stringformat:"s" %}
                checked="checked"
                {% endif %}>
                {{ choice.1 }}
            </label>
    ...
    <div class="{{ css_classes.wrap }}">
        ...
        <div class="radio">
            <label>
                <input type="radio" name="{{ field.html_name }}" id="id_{{ field.html_name }}_{{ forloop.counter }}"
                       value="{{ choice.0 }}"
                       {% if choice.0 in field.value or choice.0|stringformat:"s" in field.value or
                choice.0|stringformat:"s" == field.value|stringformat:"s" %}
                checked="checked"
